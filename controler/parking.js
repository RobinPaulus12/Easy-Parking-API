 ...
 *  responses:
 *      Parkings:
 *          description: Returns a list of all parkings with pagination
 *          content:
 *              application/json:
 *                  schema:
 *                      type: array
 ...
 *                          $ref: '#/components/schemas/Parking'
 */
export const getAllParkingsPagination = async (req, res) => {
    try {
        const page = parseInt(req.params.pagenb) || 1;
        const search = req.query.search || '';
        const limit = 6;
        const offset = (page - 1) * limit;
        const parkings = await parkingModel.getAllParkingsPagination(pool,limit,offset,search);
